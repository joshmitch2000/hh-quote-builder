...
**Structure**:
1. `window.HHQuoteNav` — imperative step navigation. Clicks Elementor's own native `.e-form__buttons__wrapper__button-next/-previous` buttons rather than reimplementing step logic. Handles forward-skip of empty Step 2 (no add-ons); backward-skip via a click listener on the native "Go back" button + `requestAnimationFrame` re-click if it lands on an empty Step 2. Skipped entirely in the Elementor editor preview (detected via the `?elementor-preview=` URL param, reliable at init time): all steps render stacked there, so navigation is meaningless and clicking native buttons would fight widget selection.
...
3. `Alpine.store('quote', {...})` registered inside `alpine:init` — single source of truth: `packages`, `coverageChoices`/`styleChoices` (slug→label maps from ACF field config, never guessed/reconstructed from slugs), `coverage`/`style`/`selectedPackage`/`addonState`, derived getters (`visiblePackages`, `total`, `addonSummaryLines`, `hasSelectedAddons`, `coverageLabel`/`styleLabel`), `formatNumber()` (always comma-formatted per explicit user preference), `selectPackage()` (sets state, triggers nav). **Late-payload pattern (load-bearing):** the store is always registered at `alpine:init` with empty placeholder data so no expression ever reads `$store.quote` as undefined, then the `#hh-quote-data` payload is applied via `Object.assign`-style property sets — synchronously on the frontend (payload is server-rendered, present at `alpine:init`), or via a one-shot MutationObserver in the editor preview (payload AJAXed in later; observer disconnects on success).
4. `Alpine.effect()` #1 — when coverage changes and the current style is no longer valid, auto-selects the first valid style for the new coverage (null only if none exist). First-valid ordering follows `styleToCoverages` key order, which is also the style-tab display order and the order `defaultStyle` is computed in PHP — all three stay consistent by construction.
...
6. `alpine:initialized` listener — injects the actual template-literal HTML into `#cards-container`, `#addons-container`, `#selection-summary-container` (if present), then calls `Alpine.initTree(el)` on each. In the Elementor editor preview (detected via the `?elementor-preview=` URL param — `window.elementor` isn't defined in the iframe yet at deferred-script time), a persistent MutationObserver on `document.documentElement` (subtree) re-runs injection: the preview renders all widgets via AJAX after page load, so containers don't exist at `alpine:initialized`, and Elementor's re-renders replace the container nodes entirely (observing the containers themselves is useless). Frontend gets no observer. See `docs/elementor-editor-and-wppusher-research.md`.
...
- First available coverage AND first valid style pre-selected on load (supersedes the earlier "style left unselected" decision — reversed at user's request, v1.2.0). Cards are visible immediately; the "Select a coverage and style" placeholder is now only a zero-valid-style fallback.
