...
6. `alpine:initialized` listener — injects the actual template-literal HTML into `#cards`, `#addons-container`, `#selection-summary` (if present), then calls `Alpine.initTree(el)` on each.
